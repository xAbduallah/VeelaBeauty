...
const ProductsSection: React.FC<ProductsSectionProps> = ({
  ...
  const selectedSortLabel =
    sortOptions.find((opt) => opt.id === selectedSort)?.label || "Feature";
      ...
      <div className="flex items-center justify-between mb-6 w-full">
        ...
        <div>
          <p className="text-[18px] leading-[24px] font-normal text-black">
            Showing <span className="font-semibold">{displayCount}</span>{" "}
            results for "{title}"
          </p>
        </div>
        ...
        <div className="flex items-center gap-6">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center justify-between gap-10 px-[18px] py-[18px] border border-[#777777] rounded-[5px] bg-white w-[163px] h-[50px]"
            >
            ...
            {/* Dropdown Menu */}
            {isSortOpen && (
              <div className="absolute right-0 mt-2 w-[163px] bg-white border border-[#777777] rounded-[5px] shadow-lg z-10">
                {sortOptions.map((option, index) => (
                  <button
                    key={option.id}
                    onClick={() => handleSortChange(option.id)}
                    className={`w-full text-left px-4 py-3 text-[18px] font-normal hover:bg-gray-50 ${
